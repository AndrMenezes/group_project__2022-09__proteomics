...
Original <- function(treatment, name="no", method="Two Sample T Test") {
  data_filtered <- data |>
    filter(Method==method, Treatment %in% c(treatment)) |>
    select(c(Protein, Gene, logFC, P_value))
...
Ampicillin_proteinDE <- tibble(nrow(P("Ampicillin")), nrow(newP("Ampicillin")), nrow(FDR("Ampicillin")))
Ampicillin <- as.numeric(Ampicillin_proteinDE)
Approaches <- c("P", "newP", "FDR")
...
# export xlsx
list_amp <- list("P" = Pdrug, "new_P" = newP_drug, "FDR" = FDR_drug)
...
Impipenem_proteinDE <- tibble(nrow(P("Impipenem")), nrow(newP("Impipenem")), nrow(FDR("Impipenem")))
Imipenem <- as.numeric(Impipenem_proteinDE)
Aproaches <- c("P", "newP", "FDR")
...
# export xlsx
list_imi <- list("P" = Pdrug, "new_P" = newP_drug, "FDR" = FDR_drug)
...
Cefotaxime_proteinDE <- tibble(nrow(P("Cefotaxime")), nrow(newP("Cefotaxime")), nrow(FDR("Cefotaxime")))
...
# export xlsx
list_cef <- list("P" = Pdrug, "new_P" = newP_drug, "FDR" = FDR_drug)
...
Ciprofloxacin_proteinDE <- tibble(nrow(P("Ciprofloxacin")), nrow(newP("Ciprofloxacin")), nrow(FDR("Ciprofloxacin")))
...
# export xlsx
list_cip <- list("P" = Pdrug, "new_P" = newP_drug, "FDR" = FDR_drug)
...
newP("Cefotaxime", "yes")$Gene

# see topGene | choose approaches and treatment
P("Ampicillin", "yes")$Protein

# Comparison between Original filtering and the newOne
intersect((P("Ciprofloxacin", "yes"))$Gene, (Original("Ciprofloxacin", "yes")$Gene))
